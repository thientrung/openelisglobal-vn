...
public class PatientManagementInfo implements Serializable {
	...
	private String employerName = "";// Dũng
	...
	private static List<IdValuePair> patientAgeUnits;
	private String speciesName;
	
	public PatientManagementInfo() {
	...
	public List<Dictionary> getAddressDepartments() {
		addressDepartments = new DictionaryDAOImpl()
				.getDictionaryEntrysByCategory("description",
						"patientClinicalDept", true);

		return addressDepartments;
	}

	public List<Dictionary> getCities() {
        // Changed to populate city select options using data from District table
	    //DistrictDAO districtDAO = new DistrictDAOImpl();
	    DictionaryDAO dictionaryDAO = new DictionaryDAOImpl();
	    addressCities = dictionaryDAO.getDictionaryEntriesByCategoryId(IActionConstants.CITY_CATEGORY_ID.toString());
	    // addressCities = districtDAO.getAllDistinctCitiesInDistrictTable();

		return addressCities;
	}

	public List<Dictionary> getDistricts() {
	    // Changed to populate district select options using data from District table
	    DistrictDAO districtDAO = new DistrictDAOImpl();
        addressDistricts = districtDAO.getAllDistinctDistrictsInDistrictTable();
	...
	public List<IdValuePair> getHealthRegions() {
		if (healthRegions == null) {
			healthRegions = DisplayListService
					.getList(ListType.PATIENT_HEALTH_REGIONS);
		}
	...
	public List<IdValuePair> getEducationList() {
		if (educationList == null) {
			educationList = DisplayListService
					.getList(ListType.PATIENT_EDUCATION);
		}
	...
	public List<IdValuePair> getMaritialList() {
		if (maritialList == null) {
			maritialList = DisplayListService
					.getList(ListType.PATIENT_MARITAL_STATUS);
		}
		return maritialList;
	}
	...
	public List<IdValuePair> getNationalityList() {
		if (nationalityList == null) {
			nationalityList = DisplayListService
					.getList(ListType.PATIENT_NATIONALITY);
		}
	...
	public void setPatientAgeUnits(List<IdValuePair> patientAgeUnitList) {
		patientAgeUnits = patientAgeUnitList;
	}

	public List<IdValuePair> getpatientAgeUnits() {
		if (patientAgeUnits == null) {
			patientAgeUnits = DisplayListService.getList(ListType.PATIENT_AGE_UNITS);
		}
