...
public class PatientSearchPopulateProvider extends BaseQueryProvider {
	...
	public void processRequest(HttpServletRequest request,
		...
		String result = null;
		// cho nay khong duoc on lam vi nationalId va externalId khong on dinh
		// khong bat buoc va la duy nhat tren form
		if (nationalId != null) {
			result = createSearchResultXML(
					patientDAO.getPatientByNationalId(nationalId), xml);
		} else if (externalId != null) {
			result = createSearchResultXML(
					patientDAO.getPatientByExternalId(externalId), xml);
		} else {
			result = createSearchResultXML(getPatientForID(patientKey), xml);
		}
		if (!result.equals(VALID)) {
			result = StringUtil
					.getMessageForKey("patient.message.patientNotFound");
			xml.append("empty");
		}
	...
	private void createReturnXML(Patient patient, StringBuilder xml) {
		...
		PatientIdentityTypeMap identityMap = PatientIdentityTypeMap
				.getInstance();

		List<PatientIdentity> identityList = PatientUtil
				.getIdentityListForPatient(patient.getId());

		XMLUtil.appendKeyValue("ID", patient.getId(), xml);
		XMLUtil.appendKeyValue("nationalID", patient.getNationalId(), xml);
		XMLUtil.appendKeyValue("ST_ID",
				identityMap.getIdentityValue(identityList, "ST"), xml);
		XMLUtil.appendKeyValue("subjectNumber",
				identityMap.getIdentityValue(identityList, "SUBJECT"), xml);
		...
		XMLUtil.appendKeyValue("mother",
				identityMap.getIdentityValue(identityList, "MOTHER"), xml);
		XMLUtil.appendKeyValue("aka",
				identityMap.getIdentityValue(identityList, "AKA"), xml);
		XMLUtil.appendKeyValue("street", person.getStreetAddress(), xml);
		XMLUtil.appendKeyValue("city",
				getAddress(person, ADDRESS_PART_VILLAGE_ID), xml);
		...
		XMLUtil.appendKeyValue("insurance",
				identityMap.getIdentityValue(identityList, "INSURANCE"), xml);
		XMLUtil.appendKeyValue("chartNumber", patient.getChartNumber(), xml);
		XMLUtil.appendKeyValue("occupation",
				identityMap.getIdentityValue(identityList, "OCCUPATION"), xml);
		XMLUtil.appendKeyValue("employerName",
				identityMap.getIdentityValue(identityList, "EMPLOYER_NAME"),
				xml);
		XMLUtil.appendKeyValue("dob", patient.getBirthDateForDisplay(), xml);
		XMLUtil.appendKeyValue("commune",
				getAddress(person, ADDRESS_PART_COMMUNE_ID), xml);
		XMLUtil.appendKeyValue("addressDept",
				getAddress(person, ADDRESS_PART_DEPT_ID), xml);
		XMLUtil.appendKeyValue("addressWard",
				getAddress(person, ADDRESS_PART_WARD_ID), xml);
		XMLUtil.appendKeyValue("addressDistrict",
				getAddress(person, ADDRESS_PART_DISTRICT_ID), xml);
		XMLUtil.appendKeyValue("motherInitial",
				identityMap.getIdentityValue(identityList, "MOTHERS_INITIAL"),
				xml);
		XMLUtil.appendKeyValue("externalID", patient.getExternalId(), xml);
		XMLUtil.appendKeyValue("education",
				identityMap.getIdentityValue(identityList, "EDUCATION"), xml);
		XMLUtil.appendKeyValue("maritialStatus",
				identityMap.getIdentityValue(identityList, "MARITIAL"), xml);
		XMLUtil.appendKeyValue("nationality",
				identityMap.getIdentityValue(identityList, "NATIONALITY"), xml);
		XMLUtil.appendKeyValue(
				"otherNationality",
				identityMap.getIdentityValue(identityList, "OTHER NATIONALITY"),
				xml);
		XMLUtil.appendKeyValue("healthDistrict",
				identityMap.getIdentityValue(identityList, "HEALTH DISTRICT"),
				xml);
		XMLUtil.appendKeyValue("healthRegion",
				identityMap.getIdentityValue(identityList, "HEALTH REGION"),
				xml);
		XMLUtil.appendKeyValue("guid",
				identityMap.getIdentityValue(identityList, "GUID"), xml);
		// Dung add age
		//XMLUtil.appendKeyValue("age", person.getAge(), xml);
		// sampleOrder.setPatientAgeUnits(
		// ObservationHistoryService.getValueForSample(
		// ObservationType.PATIENT_AGE_UNITS, sample.getId() ) );
		...
		if (patient.getId() != null) {
			...
			if (observationHistory != null) {
				XMLUtil.appendKeyValue("ageUnit",
						observationHistory.getValue(), xml);
			}
			...
			observationHistory = observationHistoryDAO
					.getObservationHistoriesByPatientIdAndTypeOnlyOne(
							patient.getId(), "9");
			if (observationHistory != null && observationHistory.getValue() != null && !observationHistory.getValue().equals("0")) {
				XMLUtil.appendKeyValue("depId", observationHistory.getValue(),
						xml);
				Dictionary dictionary = new Dictionary();
				DictionaryDAO dictionaryDAO = new DictionaryDAOImpl();
				if (observationHistory.getValue() != null) {
					dictionary = dictionaryDAO
							.getDictionaryById(observationHistory.getValue());
					XMLUtil.appendKeyValue("depName",
							dictionary.getDictEntry(), xml);
				}
			}
			// show bedNumber
			observationHistory = observationHistoryDAO
					.getObservationHistoriesByPatientIdAndTypeOnlyOne(
							patient.getId(), "7");
			if (observationHistory != null) {
				XMLUtil.appendKeyValue("bedNumber",
						observationHistory.getValue(), xml);
			}
			// show roomNumber
			observationHistory = observationHistoryDAO
					.getObservationHistoriesByPatientIdAndTypeOnlyOne(
							patient.getId(), "8");
			if (observationHistory != null) {
				XMLUtil.appendKeyValue("roomNumber",
						observationHistory.getValue(), xml);
			}
			// show Diagnosis
			observationHistory = observationHistoryDAO
					.getObservationHistoriesByPatientIdAndTypeOnlyOne(
							patient.getId(), "6");
			if (observationHistory != null) {
				XMLUtil.appendKeyValue("diagnosis",
						observationHistory.getValue(), xml);
			}
			// show SpeciesName
			observationHistory = observationHistoryDAO
                    .getObservationHistoriesByPatientIdAndTypeOnlyOne(
                            patient.getId(), "12");
			if (observationHistory != null) {
                XMLUtil.appendKeyValue("speciesName",
                        observationHistory.getValue(), xml);
            }
		}
	...
	/**
	 * Fake the unknown patient by never return whatever happens to be in last
	 * name field.
	 * 
	 * @param person
	 * @return
