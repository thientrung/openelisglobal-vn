...
public class YearNumAccessionValidator implements IAccessionNumberValidator {
	...
	public String incrementAccessionNumber(String currentHighAccessionNumber) {
		...
		int increment = Integer.parseInt(currentHighAccessionNumber
				.substring(INCREMENT_START + separatorLength));
		String incrementAsString = incrementStartingValue;
		String year = DateUtil.getTwoDigitYear();

		if (year.equals(currentHighAccessionNumber.substring(YEAR_START,
				YEAR_END))) {
	...
	public ValidationResults validFormat(String accessionNumber,
		...
		if (checkDate) {
			if (!DateUtil.getTwoDigitYear().equals(
					accessionNumber.substring(YEAR_START, YEAR_END))) {
	...
	private String getFormatPattern() {
		StringBuilder format = new StringBuilder(
				StringUtil.getMessageForKey("date.two.digit.year"));
		if (useSeparator) {
			format.append(separator);
		}
