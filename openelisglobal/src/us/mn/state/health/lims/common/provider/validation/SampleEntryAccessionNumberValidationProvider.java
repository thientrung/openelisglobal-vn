 ...
 * 
 */
public class SampleEntryAccessionNumberValidationProvider extends
	...
	public void processRequest(HttpServletRequest request,
		...
		boolean ignoreUsage = "true"
				.equals(request.getParameter("ignoreUsage"));
		...
		if (ignoreYear || ignoreUsage) {
			...
			if (result == ValidationResults.SUCCESS && !ignoreUsage) {
				if (AccessionNumberUtil.isUsed(accessionNumber)) {
					result = ValidationResults.SAMPLE_FOUND;
				} else if (!AccessionNumberUtil
						.isHadDataAccessionNumber(accessionNumber)) {
					result = ValidationResults.SAMPLE_NOT_IN_SYSTEM;
				} else
					result = ValidationResults.SAMPLE_NOT_FOUND;
			}
		} else {
			// year matters and number must not be used
			result = AccessionNumberUtil.checkAccessionNumberValidity(
		...
		case SAMPLE_NOT_FOUND:
			...
			break;
		default:
			returnData = !ignoreUsage ? AccessionNumberUtil
					.getInvalidMessage(result) : AccessionNumberUtil
