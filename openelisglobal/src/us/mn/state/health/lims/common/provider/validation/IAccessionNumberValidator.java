...
public interface IAccessionNumberValidator {
	...
	public enum ValidationResults {
		SUCCESS, SITE_FAIL, YEAR_FAIL, USED_FAIL, IS_NOT_USED_FAIL, LENGTH_FAIL, FORMAT_FAIL, PROGRAM_FAIL, REQUIRED_FAIL, PATIENT_STATUS_FAIL, SAMPLE_STATUS_FAIL, SAMPLE_FOUND, SAMPLE_NOT_FOUND, SAMPLE_NOT_IN_SYSTEM
	};
	 ...
	 * @param checkDate
	 *            -- true if there is a date element in the number which should
	 *            be checked or false otherwise If the number is one which has
	 *            already been entered into the system then it would make sense
	 *            to set this to false since when the year turns over all of the
	 *            sudden they will fail.
	 * @return One of the possible results for validation
	 * @throws IllegalArgumentException
	 */
	public ValidationResults validFormat(String accessionNumber,
			boolean checkDate) throws IllegalArgumentException;

	/**
	 * Helper method for getting an appropriate message for a validation result
	 * 
	 * @param results
	 *            -- the result for which the message is wanted
	 * @return -- the message
	 */
	...
	/**
	 * @param programCode
	 *            -- code if needed, may be null
	 * @return The next available number, may be null if one can not be
	 *         generated.
	 */
	...
	/**
	 * Get the part of the accession number which should not change. ie. for
	 * Haiti it would be the site number, for Cote d'Ivoire it would be the
	 * Program prefix
	 * 
	 * @return
	 */
