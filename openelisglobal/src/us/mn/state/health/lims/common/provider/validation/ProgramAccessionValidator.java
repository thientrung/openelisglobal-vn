...
public class ProgramAccessionValidator implements IAccessionNumberValidator {
	...
	public String incrementAccessionNumber(String currentHighAccessionNumber) {
		...
		StringBuilder builder = new StringBuilder(currentHighAccessionNumber
				.substring(PROGRAM_START, PROGRAM_END).toUpperCase());
	...
	public ValidationResults validFormat(String accessionNumber,
		...
		String programCode = accessionNumber.substring(PROGRAM_START,
				PROGRAM_END).toUpperCase();
	...
	public String getInvalidMessage(ValidationResults results) {
		...
		case USED_FAIL:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number.used");
		case PROGRAM_FAIL:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number.program");
		case FORMAT_FAIL:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number.format");
		case REQUIRED_FAIL:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number.required");
		case PATIENT_STATUS_FAIL:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number.patientRecordStatus");
		case SAMPLE_STATUS_FAIL:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number.sampleRecordStatus");
		default:
			return StringUtil
					.getMessageForKey("sample.entry.invalid.accession.number");
		}
	}
	...
	public String getNextAvailableAccessionNumber(String prefix) {
		...
		String curLargestAccessionNumber = sampleDAO
				.getLargestAccessionNumberWithPrefix(prefix);
	...
	public boolean accessionNumberIsUsed(String accessionNumber,
			String recordType) {
		boolean accessionNumberUsed = new SampleDAOImpl()
				.getSampleByAccessionNumber(accessionNumber) != null;

		if (recordType == null) {
			return accessionNumberUsed;
		}
		StatusSet statusSet = StatusService.getInstance()
	...
	/**
	 * There are many possible samples with various status, only some of which
	 * are valid during certain entry steps. This method provides validation
	 * results identifying whether a given sample is appropriate given all the
	 * information.
	 * 
	 * @param accessionNumber
	 *            the number for the sample
	 * @param recordType
	 *            initialPatient, initialSample, doublePatient (double entry for
	 *            patient), doubleSample
	 * @param isRequired
	 *            the step being done expects the sample to exist. This is used
	 *            generate appropriate results, either REQUIRED_FAIL vs
	 *            SAMPLE_NOT_FOUND
	 * @param studyFormName
	 *            - an additional
	 * @return
	...
	public ValidationResults checkAccessionNumberValidity(
		...
		boolean accessionUsed = (sampleDAO
				.getSampleByAccessionNumber(accessionNumber) != null);
		if (results == ValidationResults.SUCCESS) {
			...
			if (IActionConstants.TRUE.equals(isRequired) && !accessionUsed) {
				...
				// record Type specified, so work out the detailed response to
				// report
				if (accessionUsed) {
					if (recordType.contains("initial")) {
						if (recordType.contains("Patient")) {
							results = AccessionNumberUtil
									.isPatientStatusValid(accessionNumber,
											RecordStatus.NotRegistered);
							if (results != PATIENT_STATUS_FAIL) {
								results = matchExistingStudyFormName(
										accessionNumber, studyFormName, false);
							}
						} else if (recordType.contains("Sample")) {
							results = AccessionNumberUtil
									.isSampleStatusValid(accessionNumber,
											RecordStatus.NotRegistered);
						...
						if (recordType.contains("double")) {
							if (recordType.contains("Patient")) {
								results = AccessionNumberUtil
										.isPatientStatusValid(
												accessionNumber,
												RecordStatus.InitialRegistration);
								if (results != PATIENT_STATUS_FAIL) {
									results = matchExistingStudyFormName(
											accessionNumber, studyFormName,
											true);
								}
							} else if (recordType.contains("Sample")) {
								results = AccessionNumberUtil
					...
					if (recordType.contains("initial")) {
						results = ValidationResults.SAMPLE_NOT_FOUND; // initial
																		// entry
																		// not
																		// used
																		// is
						...
						results = ValidationResults.REQUIRED_FAIL; // double
																	// entry not
																	// existing
																	// is a
																	// problem
	...
	/**
	 * Can the existing accession number be used in the given form? This method
	 * is useful when we have an existing accessionNumber and want to ask the
	 * question.
	 * 
	 * @param accessionNumber
	 * @param existingRequired
	 *            true => it is required that there is an existing
	 *            studyFormName?
	 * @return
	 */
	private static ValidationResults matchExistingStudyFormName(
			String accessionNumber, String studyFormName,
		...
		if (existingName.equals(studyFormName)
				|| (!existingRequired && GenericValidator
						.isBlankOrNull(existingName))) {
			return SAMPLE_FOUND;
		}
		return SAMPLE_STATUS_FAIL; // the sample was entered on a different
									// form!
	}
	...
	private static String findStudyFormName(String accessionNumber) {
		...
		List<ObservationHistory> all = ohDAO.getAll(
				p,
				s,
				ObservationHistoryTypeMap.getInstance().getIDForType(
						"projectFormName"));
		String existingName = "";
		if (all.size() > 0) {
